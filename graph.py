class Graph(object):
    ...
    def __init__(self, graph_dict=None):
        ...
        if self.__is_with_loop():
            raise ValueError(
                "The graph are supposed to be without self-loop please recheck the input data!"
            )
    ...
    def add_vertex(self, vertex):
        ...
        if vertex not in self.__graph_dict:
            ...
            print(
                "The vertex %s already exists in the graph, thus it has been ignored!"
                % vertex
            )
    ...
    def add_edge(self, edge):
        ...
        if not self.__is_edge_in_graph(edge):
            ...
            print(
                "The edge %s already exists in the graph, thus it has been ignored!"
                % ([vertex1, vertex2])
            )
    ...
    def __decompose_edge(self, edge):
        ...
        if (isinstance(edge, list) or isinstance(edge, tuple)) and len(edge) == 2:
            ...
            raise ValueError(
                "%s is not of type list or tuple or its length does not equal to 2"
                % edge
            )
...
class TrafficNetwork(Graph):
    ...
    def add_destination(self, destination):
        if destination not in self.__destinations:
            ...
            print(
                "The destination %s already exists, thus has been ignored!"
                % destination
            )
